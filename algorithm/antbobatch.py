...
class ProximalExploration: 
    ...
    def pick_action(self, candidate_pool,x_central_local,score_max,threshold=15):
        """Pick action."""
        states_to_screen = []
        states_to_screen = []
        method_pred = []
        ...
        for i in range(len(candidate_pool)):
            dist=levenshtein_distance(x_central_local,candidate_pool[i])
            if dist<threshold:
                candidate_pool_.append(candidate_pool[i])
    ...
    def _propose_sequences(self, measured_sequences,score_max):
        ...
        if self.num_actions == 0:
            ...
            x_central_local=self.wt_sequence
        ...
        new_state_string, _ = self.pick_action(
            candidate_pool, x_central_local,score_max
        ) 
